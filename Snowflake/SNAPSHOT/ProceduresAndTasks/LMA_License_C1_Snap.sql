...
create or replace procedure APTTUS_DW.SNAPSHOTS.CREATE_LMA_LICENSE_C1_SNAP(FLOAT_PARAM1 FLOAT)
    ...
    var sql_command = "CREATE TABLE APTTUS_DW.SNAPSHOTS.LMA_LICENSE_C1_SNAP_" + FLOAT_PARAM1 + "_" + NEW_EPOCH_SECS + " as SELECT *, CONVERT_TIMEZONE('UTC',current_timestamp()) AS SNAP_LOAD_AT from APTTUS_DW.SF_CONGA1_1.SFLMA__LICENSE__C"
    try {
        snowflake.execute (
            {sqlText: sql_command}
            );
...
--how to call example
CALL APTTUS_DW.SNAPSHOTS.CREATE_LMA_LICENSE_C1_SNAP(select Max(_SDC_TABLE_VERSION) from APTTUS_DW.SF_CONGA1_1.SFLMA__LICENSE__C);

--DROP procedure APTTUS_DW.SNAPSHOTS.UPDATE_LMA_LICENSE_C1_SNAP(FLOAT);
create or replace procedure APTTUS_DW.SNAPSHOTS.UPDATE_LMA_LICENSE_C1_SNAP(CHAR_PARAM1 VARCHAR)
    returns string
    language javascript
    strict
    as
...
and OUT_OBJECT_TYPE = 'Snap Table'" 
    ...
    var stmt4 = snowflake.createStatement({sqlText: "select Max(_SDC_TABLE_VERSION) as SDC_TABLE_V from APTTUS_DW.SF_CONGA1_1.SFLMA__LICENSE__C"});
    ...
    try {
        var stmt = snowflake.createStatement({
            sqlText: "INSERT INTO APTTUS_DW.SNAPSHOTS.LMA_LICENSE_C1_SNAP_" + table_codes + " SELECT *, CONVERT_TIMEZONE('UTC',current_timestamp()) AS SNAP_LOAD_AT from APTTUS_DW.SF_CONGA1_1.SFLMA__LICENSE__C WHERE SYSTEMMODSTAMP > (:1)",       
            binds: [FROMDATE]
            });
...
and OUT_OBJECT_NAME = 'LMA_LICENSE_C1_SNAP_'" 
                ...
                return_value = "Created New Snap for LMA_LICENSE_C1 as LMA_LICENSE_C1_SNAP_" + table_codes;   // Return a success/error indicator.
                snowflake.execute({
                    sqlText: `insert into APTTUS_DW.SNAPSHOTS.SNAP_ACTIVITY_LOG (procedure_name, step_name) VALUES (?,?)`
                    ,binds: [procname, stepname]
                    });
    ...
    snowflake.execute({
    sqlText:"UPDATE APTTUS_DW.SNAPSHOTS.SNAPSHOT_CONTROL \
   set START_AFTER_SYSTEMMODSTAMP = (SELECT MAX(SYSTEMMODSTAMP) FROM APTTUS_DW.SNAPSHOTS.LMA_LICENSE_C1_SNAP_" + table_codes + ") \
     , LAST_SDC_EXTRACTED_AT = (SELECT MAX(_SDC_EXTRACTED_AT) FROM APTTUS_DW.SNAPSHOTS.LMA_LICENSE_C1_SNAP_" + table_codes + ") \
     , LAST_SNAPSHOT_START = (CONVERT_TIMEZONE('UTC',current_timestamp())) \
     , CURRENT_SDC_TABLE_VERSION = (SELECT MAX(_SDC_TABLE_VERSION) FROM APTTUS_DW.SNAPSHOTS.LMA_LICENSE_C1_SNAP_" + table_codes + ") \
   WHERE OUT_CATALOG = 'APTTUS_DW' \
     and OUT_SCHEMA = 'SNAPSHOTS' \
     and OUT_OBJECT_NAME = 'LMA_LICENSE_C1_SNAP_' \
     and OUT_OBJECT_TYPE = 'Snap Table'"
...
DESCRIBE procedure APTTUS_DW.SNAPSHOTS.UPDATE_LMA_LICENSE_C1_SNAP(VARCHAR);
--how to call example
CALL APTTUS_DW.SNAPSHOTS.UPDATE_LMA_LICENSE_C1_SNAP(select CURRENT_SDC_TABLE_VERSION || '_' || CURRENT_SNAPSHOT_TABLE_VERSION from APTTUS_DW.SNAPSHOTS.SNAPSHOT_CONTROL WHERE OUT_CATALOG = 'APTTUS_DW' and OUT_SCHEMA = 'SNAPSHOTS' and OUT_OBJECT_NAME = 'LMA_LICENSE_C1_SNAP_');

CREATE OR REPLACE TASK APTTUS_DW.SNAPSHOTS.LMA_LICENSE_C1_SNAP
  WAREHOUSE = APTTUS_ADMIN
  SCHEDULE = 'USING CRON 03 02 * * * America/Los_Angeles'  
AS CALL APTTUS_DW.SNAPSHOTS.UPDATE_LMA_LICENSE_C1_SNAP(select CURRENT_SDC_TABLE_VERSION || '_' || CURRENT_SNAPSHOT_TABLE_VERSION from APTTUS_DW.SNAPSHOTS.SNAPSHOT_CONTROL WHERE OUT_CATALOG = 'APTTUS_DW' and OUT_SCHEMA = 'SNAPSHOTS' and OUT_OBJECT_NAME = 'LMA_LICENSE_C1_SNAP_');
