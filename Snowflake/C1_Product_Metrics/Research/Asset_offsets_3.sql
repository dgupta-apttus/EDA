...
UNION --
        SELECT A.APTTUS_CONFIG2__ACCOUNTID__C      AS ACCOUNTID
             ...
             , COALESCE(B.FAMILY, 'Unidentified Product') AS PRODUCT_PRODUCT
             , COALESCE(E.PRODUCT,  'Unidentified Product') AS ASSET_PRODUCT 
...
--, inner1 as (
        SELECT A.ACCOUNTID
             ...
             , COALESCE(ROUND(SUM(B.ACV) OVER (PARTITION BY B.ACCOUNTID), 2), 0) AS POSITIVE_ACCOUNT_ACV  -- this is for precentage calcs only here for check your work
        FROM                    sum_product_c1_all A
        LEFT OUTER JOIN         sum_product_c1_pos B
                          ON  A.ACCOUNTID = B.ACCOUNTID
                          AND A.CRM = B.CRM
                          AND A.ASSET_PRODUCT = B.ASSET_PRODUCT             
--order by 1, 3        
--
;                 
