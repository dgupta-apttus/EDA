...
select * from tempInner
;

select CRM_SOURCE, count(*) 
from tempInner
where ACTIVE_LICENSE_COUNT > 0
group by CRM_SOURCE 


;
select CRM_SOURCE, STATUS, ORG_STATUS, IS_SANDBOX, EXPIRATION_DATE_STRING, count(*)
from tempInner
group by CRM_SOURCE, STATUS, ORG_STATUS, IS_SANDBOX, EXPIRATION_DATE_STRING
;

select * from tempInner
where LICENSE_SEAT_TYPE = 'Seats'
  and ACTIVE_SEATS = 0 
  and SEATS > 0
and CRM_SOURCE <> 'Conga 1.0'
;  