create or replace procedure APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES(MODE VARCHAR)
    returns string
    language javascript
    strict
    as
    $$
    var procname = "FILL_MONTHLY_ACTIVITY_SCORES"
    var MODE_INNER = MODE
    var set_run = `
...
// execute SQLs
    ...
    var stepname = "Insert MONTHLY_ACTIVITY_SCORES from MONTHLY_ACTIVITY"  
    try {
        snowflake.execute (
            {sqlText: sql_command}
            );
        ...
        if (DUP_COUNT != 0){
			return_value = "Rows Inserted to MONTHLY_ACTIVITY_SCORES but duplicate values found"
            error_code = 1
            snowflake.execute (
...
DESCRIBE procedure APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES(varchar);
--how to call example
CALL APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES('Full'); -- the other option is manual     
-- to run in manual mode
-- first if they exist already delete all records from APTTUS_DW.PRODUCT.MONTHLY_ACTIVITY_SCORES that have a month you want to replace or rerun
...
---- --DELETE FROM MONTHLY_ACTIVITY_SCORES WHERE REPORT_DATE = '2035-08-01';  
CALL APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES('Manual');

CREATE OR REPLACE TASK APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES
  WAREHOUSE = APTTUS_ADMIN
  SCHEDULE = 'USING CRON 50 04 1 * * America/Los_Angeles' -- on first day of month
AS CALL APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES('Full')
; 
 
DESCRIBE task APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES;
alter task APTTUS_DW.PRODUCT.FILL_MONTHLY_ACTIVITY_SCORES suspend; --resume
