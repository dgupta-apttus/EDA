create or replace procedure APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY(MODE VARCHAR)
    returns string
    language javascript
    strict
    as
    $$
    var procname = "FILL_LMA_LIC_PACKAGE_MONTHLY"
    var MODE_INNER = MODE
    var set_run = `
...
// execute SQLs
    ...
    var stepname = "Insert LMA_LIC_PACKAGE_MONTHLY"  
    try {
        snowflake.execute (
            {sqlText: sql_command}
            );
        ...
        if (DUP_COUNT != 0){
			return_value = "Rows Inserted to LMA_LIC_PACKAGE_MONTHLY but duplicate values found"
            error_code = 1
            snowflake.execute (
...
DESCRIBE procedure APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY(varchar);
--how to call example
CALL APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY('Full'); -- the other option is manual     
...
-- first if they exist already,  delete all records from APTTUS_DW.PRODUCT.LMA_LIC_PACKAGE_MONTHLY for the recently completed month
...
---- --DELETE FROM APTTUS_DW.PRODUCT.LMA_LIC_PACKAGE_MONTHLY WHERE REPORT_DATE = '2035-08-01';  
CALL APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY('Manual');

CREATE OR REPLACE TASK APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY
  WAREHOUSE = APTTUS_ADMIN
  SCHEDULE = 'USING CRON 16 02 1 * * America/Los_Angeles' -- after LMA snapshots
AS CALL APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY('Full')
; 
 
DESCRIBE task APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY;
alter task APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY suspend; --resume
alter task APTTUS_DW.PRODUCT.FILL_LMA_LIC_PACKAGE_MONTHLY resume;
